class BoardMessagesController < ApplicationController
  ...
  def index
    @board_messages = BoardMessage.page(params[:page])
    ...
    if users[0] == nil then
      ...
      user.save
      users = BoardUser.where 'account_it == ?', current_account.id
    end
    @board_user = users[0]
  ...
  def create
    @board_message = BoardMessage.page(params[:page])
      .order('created_at desc')
  ...
  private
    ...
    def board_message_params
      params.require(:board_message).permit(:message, :board_user_id)
    end
end
